...
$$\mathfrak{L}(\theta, \phi; X) = \Sigma^M_{n=1} \mathfrak{L}(\theta, \phi; \mathb{x}^{(n)}|\mathb{\tilde{\mu}}_2) + const \\
\mathfrak{L}(\theta, \phi; \mathb{x}^{(n)}) = \mathfrak{L}(\theta, \phi; \mathb{x}^{(n)}|\mathb{\tilde{\mu}}_2) + \frac{1}{N}log p_{\theta}(\mathb{\tilde{\mu}}_2) + const$$
...
$$\mathfrak{L}^{dis}(\theta, \phi; \mathb^{(i, n)}) = \mathfrak(\theta, \phi; \mathb^{(i, n)}) + \alpha log p(i|\mathb{z}_2^{(i, n)})\\
log p(i|\mathb{z}_2^{(i, n)}) := log p_{\theta}(\mathb{z}_2^{(i, n)}|\mathb{\tilde{\mu}}_2^{(i)}) - log(\Sigma_{j=1}^{M}p_{\theta}(\mathb{z}_2^{(i, n)}|\mathb{\tilde{\mu}}_2^{(j)})$$
