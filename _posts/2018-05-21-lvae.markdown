...
이때 reconstruction시 z를 잘 활용하지 않는 문제를 해결하기 위하여 KLdivergence를 처음에는 적게 주고 넘차 올리는 warm-up방식을 활용한다. 
Skip RNN은 State의 update를 skip하면서 계산량을 줄이면서 성능을 유지할 수 있다. 각 State를 skip할 것인지 여부는 state update gate u를 통하여 결정한다. 또한 batch normalization을 활용하면 좋다. 
